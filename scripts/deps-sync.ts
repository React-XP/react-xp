 ...
 */
const packages = [{ path: 'src/packages/console', name: '@react-xp/console', version: 'auto' }];
...
const getVersions = async () => {
	...
	return new Promise<object>((resolve, reject) => {
		...
		for (const package_ of packages) {
			exec(`npm view ${package_.name} versions --json`, (error, stdout, stderr) => {
				if (error) {
					errorCount++;
...
const syncDeps = async () => {
	...
	for (const package_ of packages) {
		(async () => {
			...
			fs.copyFileSync(`${packagePath}/package.json`, `${packagePath}/package.json.bak`);
			...
			try {
				...
				for (const peerDep in packageJson.peerDependencies) {
					if (commonPrdDeps[peerDep] && commonPrdDeps[peerDep] !== packageJson.peerDependencies[peerDep]) {
						packageJson.peerDependencies[peerDep] = commonPrdDeps[peerDep];

						logger(2, `--> Updated peer dependency ${peerDep} to ${commonPrdDeps[peerDep]}`);
					}
				}

				for (const devDep in packageJson.devDependencies) {
					if (versions[devDep] && versions[devDep] !== packageJson.devDependencies[devDep]) {
						packageJson.devDependencies[devDep] = versions[devDep];

						logger(2, `--> Updated dev dependency ${devDep} to ${versions[devDep]}`);
					}

					if (commonPrdDeps[devDep] && commonPrdDeps[devDep] !== packageJson.devDependencies[devDep]) {
						packageJson.devDependencies[devDep] = commonPrdDeps[devDep];

						logger(2, `--> Updated dev dependency ${devDep} to ${commonPrdDeps[devDep]}`);
					}

					if (commonDevDeps[devDep] && commonDevDeps[devDep] !== packageJson.devDependencies[devDep]) {
						packageJson.devDependencies[devDep] = commonDevDeps[devDep];

						logger(2, `--> Updated dev dependency ${devDep} to ${commonDevDeps[devDep]}`);
					}
				}
				...
				fs.copyFileSync(`${packagePath}/package.json.bak`, `${packagePath}/package.json`);
	...
	exec('npm run prettier-format-all-package-json', (error, stdout, stderr) => {});
